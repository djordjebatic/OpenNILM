...
def get_abs_error(target, prediction):
    ...
    if not isinstance(target, float) and not isinstance(prediction, float):

        assert (target.shape == prediction.shape), '{}, {}'.format(target.shape, prediction.shape)
...
def get_statistics(data):
    ...
    mean = np.mean(data)
    #print(np.average(np.average(data, weights=None, axis=0)))
    std = np.std(data)
